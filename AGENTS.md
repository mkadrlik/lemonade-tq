...
Custom Lemonade SDK inference server with **ROCm TurboQuant** for long-context inference and **Vulkan** for speed. Fetches llama.cpp binaries from ghcr.io at startup.
...
**ROCm = TurboQuant + long context.** The only backend that supports TurboQuant KV cache compression (3-bit, 5.12x reduction).

**Vulkan = speed.** Faster token generation for standard workloads. Requires `-fa on` (Flash Attention).

**CPU = deprecated.** Never tested, no TurboQuant benefit, no purpose.
